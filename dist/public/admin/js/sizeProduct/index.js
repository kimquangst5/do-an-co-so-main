const main = () => {
  ...
  btnCreateColorProduct.addEventListener("click", () => {
    ...
    icon.addEventListener("click", () => {
      questionYesNo(
        'warning', 'Xóa kích thước', 'Bạn có chắc muốn xóa kích thước này?', 'Xóa', '#4BC18F', 'Hủy', '#FFA09B',
        () => {
  ...
  btnUpdateColorProduct.addEventListener("click", () => {
    questionYesNo(
      'warning', 'Cập nhật kích thước', 'Bạn có chắc muốn cập nhật kích thước này?', 'Cập nhật', '#4BC18F', 'Hủy', '#FFA09B',
      () => {
        const listItem = document.querySelectorAll("[items-size-product]");
        const listData = [];
        listItem.forEach((it) => {
