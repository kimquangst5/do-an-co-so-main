const deleteProduct = () => {
     ...
     listItem.forEach((it) => {
          ...
          btnTrash.addEventListener("click", () => {
               ...
               questionYesNo(
                    'warning', 'Xóa vĩnh viễn?', `Bạn chắc muốn xóa sản phẩm ${name.innerHTML}?\nLưu ý: Hành động này không thể khôi phục!`, 'Xóa', '#4BC18F', 'Hủy', '#FFA09B',
                    () => {
...
const changeStatusMany = async () => {
     ...
     const dropdown = document.querySelector("sl-dropdown[delete]");
     if (!dropdown) return;
     const link = dropdown.getAttribute("delete");
     if (!link) return;
          ...
          if (itemChecked.length == 0) {
               ...
               if (value == "trash-product") {
                    Swal.fire({
                         ...
                         title: `Xóa vĩnh viễn?`,
                         text: `Bạn chắc muốn xóa vĩnh viễn ${itemChecked.length} sản phẩm đã chọn?\nLưu ý: Hành động không thể khôi phục!`,
                         icon: "warning",
...
const restoreProduct = () => {
     ...
     if (listItem.length <= 0) return;
     listItem.forEach((it) => {
          const btnTrash = it.querySelector("button[restore]");
          const name = it.querySelector("[name-product]");
          btnTrash.addEventListener("click", () => {
               questionYesNo(
                    'warning', 'Khôi phục sản phẩm?', `Bạn chắc muốn khôi phục sản phẩm?`, 'Khôi phục', '#4BC18F', 'Hủy', '#FFA09B',
                    () => {
                         const link = btnTrash.getAttribute("restore");
                         console.log(link);
